...
public class SugiyamaLayoutAlgorithm implements LayoutAlgorithm {
	...
	public void perform(
		...
		// Maps an Object from the graphCells list to a Cell View
	...
	protected void updateProgress4Movements() {
		...
		if (movements.size() > 1) {
			...
			for (int i = 0; i < movements.size(); i++) {
				sumMovements += ((Integer) movements.get(i)).intValue();
			}
			float averageMovements = sumMovements / movements.size();
			float gradient =
				(averageMovements - movementsMax) / (movements.size());
			float iterationsTill0 = -movementsMax / gradient;
	...
	protected void solveEdgeCrosses(JGraph jgraph, Vector levels) {
		dlgProgress.setMessage("solving cross over points ...");
	...
	protected int moveToBarycenter(
		...
		for (int currentIndexInTheLevel = 0;
			...
			for (int i = 0; i < portCount; i++) {
				...
				int minLevelDistance = Integer.MAX_VALUE;
				Vector nearestWrappers = null;
	...
	protected void drawGraph(
		...
		for (int rowCellCount = 0;
			...
			Enumeration enumLevel = level.elements();
			...
			for (int colCellCount = 0;
				...
				Rectangle bounds = (Rectangle) view.getBounds().getBounds().clone();
